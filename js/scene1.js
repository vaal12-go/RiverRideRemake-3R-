class Example extends Phaser.Scene {
  ...
  create() {
    ...
    this.main_tile_plate = this.add
    .renderTexture(0, 0, this.game.config.width, this.game.config.height*3)
    .setOrigin(0);
    ...
    var scr1 = this.fillArray(50)
    this.screenTilesArrays.push(scr1)

    var scr2 = this.fillArray(56)
    this.screenTilesArrays.push(scr2)

    var scr3 = this.fillArray(42)
    this.screenTilesArrays.push(scr3)

    console.log('this.SCENE_ROW_NO :>> ', this.SCENE_ROW_NO);
    console.log('this.game.config.height :>> ', this.game.config.height);
    console.log('this.SCENE_ROW_LEN :>> ', this.SCENE_ROW_LEN);

    for(var scr=0; scr<this.screenTilesArrays.length; scr++) {
      console.log('scr*this.game.config.height :>> ', scr*this.game.config.height);
      for(var y=0; y<this.SCENE_ROW_NO;y++) {
        for (var x=0;x<this.SCENE_ROW_LEN;x++) {
          this.main_tile_plate.draw(
                this.tileArr[this.screenTilesArrays[scr][y][x]], x*32, 
